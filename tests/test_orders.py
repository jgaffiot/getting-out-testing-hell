...
def setup_test_data():
    ...
    user_resp = requests.post(f"{BASE_URL}/users/", json={"email": "tester@example.com", "name": "Tester"})
    _user_id = user_resp.json()["id"]
    book_resp = requests.post(f"{BASE_URL}/books/", json={"title": "Test Book", "author": "Author", "price": 20.0, "stock": 5})
...
def test_create_order_success():
    ...
    response = requests.post(f"{BASE_URL}/orders/", params={"card_token": "tok_visa"}, json={
        "user_id": _user_id,
